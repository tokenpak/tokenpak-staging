...
- **Context compression** — deterministic token reduction on real agent
  workloads with local, route-specific overhead. Savings are route-specific: direct API, CLI, and
...
- **MultiPak OSS surface** — read-only Vault Pak adapter, companion journal promotion-candidate marking, `tokenpak pak` CLI, OSS PakRank Lite via `tokenpak pakplan preview`, and `/pak/v1/*` proxy stubs. Full MultiPak automation (capture pipeline, advanced ranking overlays, Handoff Paks, anchor hydration, Context Package assembly) requires `tokenpak-paid` (Pro). See [docs/multipak.md](docs/multipak.md).
