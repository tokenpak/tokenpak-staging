...
This was not always true. Earlier builds imported `sentence_transformers` at
module load, which transitively pulled in `torch` — a cold import of roughly
13–18 seconds. That delay could exceed Claude Code's MCP-connect window, so the
client reported the server as a failed setup ("⚠ N setup issues: MCP") even
though nothing was actually broken.

### The durable fix (already in place)

The retrieval backend is now lazy-loaded. Availability is *detected* cheaply at
import (no `torch`), and the model is imported only on first retrieval. This is
the durable resolution: a fresh `tokenpak claude` / `tokenpak codex` connects
without tripping the timeout, and the setup-issue banner no longer appears. No
configuration is required to get this behavior — it is the default.

### Safe workaround (rarely needed now)

If you are on a constrained machine, a cold filesystem cache, or a very tight
client timeout and you still see an MCP connect timeout on the very first
launch, raise Claude Code's MCP startup timeout for that session:
...
Codex the equivalent cushion (`startup_timeout_sec = 30`) is already written
into the config block above, so no manual step is needed there.
...
- [Onboarding Guide](onboarding.md) — Day 1 → Day 30 journey.
