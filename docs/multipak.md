...
| `tokenpak pak import <pak-file> [--name <name>]` | ✅ OSS | Installs an exported Pak file into the local store |
...
Vault Paks: `vault:<source-path>#<sha256-prefix>` — for example, `vault:/projects/demo/README.md#abc12345`. The `#` is significant — when used in URLs (e.g., `/pak/v1/inspect/<pak-id>`), percent-encode it as `%23`.
