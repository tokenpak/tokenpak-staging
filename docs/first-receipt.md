...
receipt. It requires a provider API key because the receipt comes from a real
eligible provider request, not from `tokenpak demo`.
...
Expected artifact: `~/.tpk/monitor.db`, table `requests`.
...
tokens, and `cache_origin` attribution (`proxy`, `client`, or `unknown`).
...
The receipt is created only after a real request passes through the local proxy.
...
No TokenPak cloud service, paid daemon, multi-instance service, dashboard write,
