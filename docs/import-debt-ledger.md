# Import-contract debt ledger (Std 10 A4 / Std 01 §2)

**Status:** DECLARED DEBT — every row below is a real import edge on staging that
violates the Std 01 architecture contract and is explicitly ignored (exact,
...
- **Ruling:** Option B per `p1-v113-a4-opus48-debt-ratchet-ruling-2026-07-19`
  (Suki, Opus 4.8 independent). Declared-not-silent.
- **Baseline row count:** **71**. This ledger is **monotonically shrinking**:
  ...
  PR that removes the physical import). Adding a row requires a Kevin-signed
  Std 10 §3 waiver (`DEBT_AUTHORITY: KEVIN`). Enforced by
  `tests/lint/test_import_contracts.py`.
- **Debt acceptance authority:** each row carries a Kevin sign-off placeholder.
...
**Std 01 §5.2 exception set: EMPTY.** §5.2 sanctions exactly one shape —
*entrypoint → `services/` direct calls* under conditions A (byte-preservation),
B (proxy infeasible), or C (pure local helper). None of the 71 edges has that
shape (they are subsystem→subsystem upward imports, monolith couplings, or
entrypoint-sibling imports). Therefore no edge here is reclassified as a §5.2
exception, and no `# tokenpak: §5.2-exception` call-site markers are added by
this change. Any future claim that a row qualifies must name the condition and
add the module-docstring citation per Std 01 §5.2 enforcement.

## Burn-down order (ruled)
...
contract; its slot in the order is a Suki decision — suggested alongside
Group 3 (same entrypoint-surface inversion work). Minimal-split recipes per
group live in
`~/vault/06_RUNTIME/evidence/cali/p1-v113-a4-import-contract-enforcement-2026-07-19/a4-contract-violation-analysis.md`.
...
| ID | Edge (exact, as in `.importlinter`) | Class | Kevin sign-off | Tracker | Src |
|---|---|---|---|---|---|
...
| ID | Edge | Class | Kevin sign-off | Tracker | Src |
|---|---|---|---|---|---|
...
mount their routes and helpers. Std 01 §1 direction is the reverse —
...
01 §10 debt item D1 territory. Closing tracker: `BURN-A4-G3`.
...
01 §1 canonical measurement ownership (moving it inverts 3 edges); shadow
...
Rationale: surfaced by the sibling-independence contract (post-ruling, same
staging base). One entrypoint importing another directly violates Std 01 §1
entrypoint independence; shared logic belongs in `services/` or a lower
primitive (Std 01 §1.3 invariant 4). Sequencing not covered by the ruled
burn-down order — Suki to slot (suggested: with Group 3). Closing tracker:
`BURN-A4-G8`.
...
*Maintained under `p1-v113-a4-import-contract-enforcement-2026-07-19`. Do not
edit by hand without updating `.importlinter` in the same commit — the
