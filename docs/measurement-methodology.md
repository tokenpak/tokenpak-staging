...
| Cache-attributed savings | Full input price for tokens that would otherwise be sent uncached, compared with the provider's cached-input price. | Cache-token counts come from provider usage fields when present. Cost uses the applicable input and cache rates. Provider/platform cache savings remain labelled separately and are not credited to TokenPak. | Sum by attribution source; cache hit rate is cache-read tokens divided by total input plus cache-read tokens. | Providers may omit detailed cache usage, and a model may lack a verified cache price. In either case token attribution or the dollar estimate is reported as unavailable instead of being invented. |
