...
from tokenpak.proxy.server_async import _parse_sse_tokens
...
def test_codex_response_completed_usage_extracts_model_tokens_and_cache_read():
    ...
    usage = _codex_response_completed_usage(payload)
    ...
    assert _parse_sse_tokens(payload) == {
        "output_tokens": 45,
...
def test_codex_usage_ignores_non_completed_response_events():
    payload = (
        b"event: response.created\n"
        b'data: {"response":{"model":"wrong","usage":{"input_tokens":999}}}\n\n'
    )

    assert _codex_response_completed_usage(payload) == {
        "model": "",
        "input_tokens": 0,
        "output_tokens": 0,
        "cache_read_tokens": 0,
