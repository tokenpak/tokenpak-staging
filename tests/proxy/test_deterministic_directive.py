...
"""Tests for the ``[TIP: deterministic=on]`` directive (PROP4-1B; Std 31 draft).

Coverage map (per packet P1-PROP4-1B-STD31-DETERMINISTIC-DIRECTIVE):
...
class TestRegistryAndParse:
    ...
    def test_unsupported_value_marks_fail_loud_never_silent(self, value):
        ...
        not silently dropped (PROP4-1B fail-loud contract)."""
        d, _ = parse_tip_header(f"[TIP: deterministic={value}] hello")
        assert d is not None
        assert d.deterministic is False
