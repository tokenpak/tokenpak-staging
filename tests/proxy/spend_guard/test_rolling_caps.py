...
def test_rolling_caps_resolve_canonical_monitor_db_from_paths(tmp_monitor_db):
    """Default lookup uses tokenpak._paths.monitor_db(mode="read"), not a
    hardcoded legacy ~/.tokenpak path.
    """
    record_session_agent("agent-a-canonical", "agent-a")
    insert_request(tmp_monitor_db, "agent-a-canonical", cost=2.0, seconds_ago=60)

    usage = compute_rolling_usage("agent-a", 3600)

    assert usage["agent_cost_usd"] == pytest.approx(2.0)
    assert usage["degraded"] is False
    assert usage["degraded_reason"] is None


def test_rolling_caps_missing_monitor_db_warns_and_marks_degraded(tmp_path, capsys):
    missing = tmp_path / "missing-monitor.db"
    rc.reset_caches_for_testing()

    usage = compute_rolling_usage("agent-a", 3600, monitor_db_path=str(missing))

    captured = capsys.readouterr()
    assert "WARN" in captured.err
    assert "monitor.db not found" in captured.err
    assert str(missing) in captured.err
    assert usage["agent_cost_usd"] == 0.0
    assert usage["degraded"] is True
    assert usage["degraded_reason"] == "monitor_db_missing"


def test_rolling_caps_broken_monitor_db_warns_and_marks_degraded(tmp_path, capsys):
    broken = tmp_path / "broken-monitor.db"
    conn = sqlite3.connect(str(broken))
    conn.execute("CREATE TABLE not_requests (id INTEGER PRIMARY KEY)")
    conn.commit()
    conn.close()
    rc.reset_caches_for_testing()

    usage = compute_rolling_usage("agent-a", 3600, monitor_db_path=str(broken))

    captured = capsys.readouterr()
    assert "WARN" in captured.err
    assert "monitor.db query failed" in captured.err
    assert str(broken) in captured.err
    assert usage["fleet_tokens_total"] == 0
    assert usage["degraded"] is True
    assert usage["degraded_reason"] == "monitor_db_query_failed"
