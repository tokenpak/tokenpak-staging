...
Lane 1 UX onboarding (p0-tokenpak-ux-lane1-stop-the-bleeding) — covers
the ``--no-tui`` escape hatch at every TTY entry point per AC #4:
...
# ---------------------------------------------------------------------------
# --no-tui on `tokenpak setup`
# ---------------------------------------------------------------------------


def test_setup_with_no_tui_takes_print_path(monkeypatch, capsys, tmp_path):
    """`tokenpak setup --no-tui` with no env keys → print-only, no run_menu."""
    ...
    _cli_core.cmd_setup(argparse.Namespace())
