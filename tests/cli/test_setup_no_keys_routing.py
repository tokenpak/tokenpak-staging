...
Covers ``tokenpak setup`` routing when no provider API keys
are present in the environment.

Previous behavior: cmd_setup printed a single "set ANTHROPIC_API_KEY"
hint and returned, dead-ending the user. New routing:
...
def test_no_keys_non_tty_prints_deterministic_instructions(clean_env, monkeypatch, capsys):
    ...
    out = capsys.readouterr().out
    # deterministic, no prompt, both paths covered
    assert "Path A" in out and "Path B" in out
    # Claude Code OAuth-passthrough copy (phrasing may wrap across lines)
    assert "no API key required" in out
    assert "OAuth" in out and "byte-preserved" in out
    # shell-aware rendering — posix uses export NAME='...'
...
def test_no_keys_non_tty_rendering_is_stable(clean_env, monkeypatch, capsys):
    """same env → same output across runs (modulo timestamps; we have none)."""
    _patch_isatty(monkeypatch, stdin=False, stdout=False)
    monkeypatch.setenv("TOKENPAK_SHELL", "posix")
...
# ---------------------------------------------------------------------------
# Shell-aware rendering branches
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# no regression when env keys ARE present
# ---------------------------------------------------------------------------
