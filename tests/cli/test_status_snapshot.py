...
"""Extraction-equivalence tests: the ``menu_status`` shim re-exports the proxy
probe that now lives in ``tokenpak.status.snapshot``, byte-for-byte behavior
preserved, and the new provider package adds no public-API surface.

Guards architecture-contract §5 ("extract ``StatusCache``/``ProxyStatus`` from
``menu_status``") + the public-API snapshot stability constraint.
"""
