...
def _run_doctor_json(monkeypatch, tmp_path, health):
    ...
    cli_mod = types.ModuleType("tokenpak.cli")
    cli_mod.__path__ = []  # type: ignore[attr-defined]
    commands_mod = types.ModuleType("tokenpak.cli.commands")
    commands_mod.__path__ = []  # type: ignore[attr-defined]
    permissions_mod = types.ModuleType("tokenpak.cli.commands.permissions")
    permissions_mod.doctor_rows = lambda: ([], False)
    monkeypatch.setitem(sys.modules, "tokenpak.cli", cli_mod)
    monkeypatch.setitem(sys.modules, "tokenpak.cli.commands", commands_mod)
    monkeypatch.setitem(sys.modules, "tokenpak.cli.commands.permissions", permissions_mod)
