...
The prior slim-wheel guard (``test_dispatch_slim_wheel_guard.py``, PR #282) covers
the *runtime-absent* case: the released wheel ships the CLI + registry data but not
the runtime engine, so a runtime verb degrades to a "source/main-only" notice.
...
``jsonschema``) are NOT installed. Those deps are kept out of the slim core
(Std 02 §9 / Constitution 00), so a base install can reach this state.
...
def test_runtime_absent_still_reports_source_only(monkeypatch):
    ...
    checked FIRST, so a genuinely-absent runtime still yields the B1/B2 message
    even if deps also happen to be missing.
    """
