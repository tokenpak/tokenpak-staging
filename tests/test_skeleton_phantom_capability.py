...
When Phase 2 (``p2-skeleton-extractor-implementation``) lands the real
extractor, the ``pytest.skip`` guards below deactivate the "absent" assertions
and that packet replaces them with their post-implementation inverse.
"""
...
pytestmark = pytest.mark.skipif(
    _extractor_present(),
    reason="skeleton extractor present (Phase 2 landed); absence-invariant tests N/A",
)
...
def test_extractor_missing_preserves_code_body(monkeypatch):
    ...
    pre-existing cosmetic quirk left for Phase 2 (the real extractor). This test
