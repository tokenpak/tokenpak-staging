...
def _write_tree(
    root: Path, *, overrides: dict[str, str] | None = None, drop: tuple[str, ...] = ()
) -> None:
...
def test_enforcing_mode_rejects_retired_public_claim(tmp_path):
    _write_tree(
        tmp_path,
        overrides={
            "README.md": (_CONFORMANT_FILES["README.md"] + "\nSame results, fewer tokens.\n"),
        },
    )
    ...
    assert result.returncode == 1
    assert any(
        finding["severity"] == "ERROR" and finding["check"] == "claim-regression"
        for finding in payload["findings"]
    )
