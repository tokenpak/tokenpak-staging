"""Std 10 A4 architecture-contract gate tests (p1-v113-a4).

Four surfaces, per the Suki ruling (Option B, debt-ratchet):
...
# Ruled baseline (Suki ruling B, 2026-07-19): 71 declared debt edges.
# This number may only DECREASE. Increasing it requires a Kevin-signed
# Std 10 §3 waiver (DEBT_AUTHORITY: KEVIN) recorded in the ledger.
DEBT_BASELINE = 71
...
def test_gate_is_green_on_committed_config():
    ...
    assert result.returncode == 0, (
        ...
        f"extend ignore_imports without a Kevin-signed waiver) or a ledgered "
...
def test_debt_is_monotonically_shrinking():
    ...
    assert len(ledger) <= DEBT_BASELINE, (
        ...
        f"debt requires a Kevin-signed Std 10 §3 waiver (DEBT_AUTHORITY: "
        f"KEVIN); new import edges must be fixed, not ledgered."
    )
...
def test_services_package_fully_covered():
    ...
    assert (services_dir / "__init__.py").exists(), (
        "tokenpak/services/__init__.py missing — grimp is blind to services "
        "modules without it (SERVICES_INIT: REQUIRE, Suki ruling 2026-07-19)"
    )
