...
Pins stub #1 of the silent-failure-surfaces coverage lane
(`p0-regression-coverage-waveC-cli-licensing`). Restores coverage for
``tokenpak/cli/cli_doctor.py::cmd_doctor`` after the original
``tests/test_doctor.py`` was deleted (commit ``a29b8e71ed``) and never
replaced (``test_doctor_claude_code.py`` exercises a *different* module).
...
def test_unmet_python_version_exits_1(doctor_home, monkeypatch):
    ...
    _stub_health(monkeypatch, _HEALTHY_HEALTH)
    monkeypatch.setattr(
        cli_doctor.sys, "version_info", _FakeVersionInfo(3, 9, 0, "final", 0)
    )
    with pytest.raises(SystemExit) as exc:
        cli_doctor.cmd_doctor(_args())
    assert exc.value.code == 1
