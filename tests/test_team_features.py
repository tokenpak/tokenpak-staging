...
class TestAgentRecord:
    def test_to_dict_includes_seconds_since_heartbeat(self):
        rec = AgentRecord(name="gamma", last_heartbeat=time.time() - 5)
        d = rec.to_dict()
        assert d["seconds_since_heartbeat"] >= 5.0
        assert d["name"] == "gamma"

    def test_roundtrip_serialisation(self):
        rec = AgentRecord(name="beta", capabilities=["compression"], status="online")
        d = rec.to_dict()
...
class TestAgentRegistry:
    ...
    def test_register_creates_agent(self):
        reg = self._registry()
        agent = reg.register("gamma", capabilities=["compression", "tools"])
    ...
    def test_heartbeat_updates_timestamp(self):
        reg = self._registry()
        reg.register("beta")
        before = reg.get("beta").last_heartbeat
        time.sleep(0.01)
        reg.heartbeat("beta")
        after = reg.get("beta").last_heartbeat
        assert after > before
    ...
    def test_list_agents_dict_serialisable(self):
        reg = self._registry()
        reg.register("gamma", capabilities=["tools"])
        dicts = reg.list_agents_dict()
        assert isinstance(dicts, list)
        assert dicts[0]["name"] == "gamma"
    ...
    def test_get_v1_agents_response_format(self):
        ...
        reg = self._registry()
        reg.register("beta", capabilities=["vault"])
...
def _make_block(block_id: str, contributor: str = "beta", path: str = "foo.py") -> SharedVaultBlock:
...
class TestSharedVault:
    ...
    def test_push_and_pull_blocks(self):
        vault = self._vault()
        block = _make_block("beta:foo.py#abc123")
        vault.push_block(block)
        blocks = vault.pull_blocks()
        assert len(blocks) == 1
        assert blocks[0].block_id == "beta:foo.py#abc123"

    def test_pull_by_contributor(self):
        vault = self._vault()
        vault.push_block(_make_block("beta:a.py#1", contributor="beta"))
        vault.push_block(_make_block("gamma:b.py#2", contributor="gamma"))
        beta_blocks = vault.pull_blocks(contributor="beta")
        assert len(beta_blocks) == 1
        assert beta_blocks[0].contributor == "beta"

    def test_merge_local_takes_priority(self):
        vault = self._vault()
        # Team has block at path foo.py
        vault.push_block(_make_block("beta:foo.py#team", path="foo.py"))
    ...
    def test_merge_team_fills_missing_paths(self):
        vault = self._vault()
        vault.push_block(_make_block("beta:bar.py#team", path="bar.py"))
    ...
    def test_delete_block(self):
        vault = self._vault()
        block = _make_block("beta:x.py#abc")
        vault.push_block(block)
        assert vault.delete_block("beta:x.py#abc") is True
        assert vault.get_block("beta:x.py#abc") is None
...
class TestTemplate:
    ...
    def test_roundtrip_serialisation(self):
        t = Template(name="test", content="{{x}}", created_by="alpha", tags=["qa"])
        d = t.to_dict()
...
class TestTemplateStore:
    ...
    def test_create_and_get(self):
        store = self._store()
        store.create(
            "summarise", "Summarise: {{content}}", created_by="alpha", actor_role=ROLE_ADMIN
        )
    ...
    def test_create_duplicate_raises(self):
        store = self._store()
        store.create("dup", "content", created_by="alpha", actor_role=ROLE_ADMIN)
        with pytest.raises(ValueError):
            store.create("dup", "content2", created_by="alpha", actor_role=ROLE_ADMIN)
    ...
    def test_list_templates_visible_to_member(self):
        ...
        store.create(
            "secret", "secret", created_by="alpha", actor_role=ROLE_ADMIN, role_required=ROLE_ADMIN
        )
    ...
    def test_list_templates_admin_sees_all(self):
        store = self._store()
        store.create("public", "content", created_by="alpha", actor_role=ROLE_ADMIN)
        store.create(
            "secret", "secret", created_by="alpha", actor_role=ROLE_ADMIN, role_required=ROLE_ADMIN
        )
    ...
    def test_use_admin_only_template_as_member_raises(self):
        store = self._store()
        store.create(
            "priv", "secret", created_by="alpha", actor_role=ROLE_ADMIN, role_required=ROLE_ADMIN
        )
        with pytest.raises(PermissionError):
    ...
    def test_delete_template(self):
        store = self._store()
        store.create("todelet", "bye", created_by="alpha", actor_role=ROLE_ADMIN)
    ...
    def test_delete_non_admin_raises(self):
        store = self._store()
        store.create("keep", "content", created_by="alpha", actor_role=ROLE_ADMIN)
        with pytest.raises(PermissionError):
            store.delete("keep", actor_role=ROLE_MEMBER)

    def test_stats(self):
        store = self._store()
        store.create("p1", "content", created_by="alpha", actor_role=ROLE_ADMIN)
        store.create(
            "p2", "content", created_by="alpha", actor_role=ROLE_ADMIN, role_required=ROLE_ADMIN
        )
        stats = store.stats()
        assert stats["total"] == 2
    ...
    def test_list_templates_filter_by_tag(self):
        store = self._store()
        store.create("t1", "content", created_by="alpha", actor_role=ROLE_ADMIN, tags=["qa", "dev"])
        store.create("t2", "content", created_by="alpha", actor_role=ROLE_ADMIN, tags=["prod"])
    ...
    def test_update_template(self):
        store = self._store()
        store.create("upd", "old content", created_by="alpha", actor_role=ROLE_ADMIN)
    ...
    def test_update_non_admin_raises(self):
        store = self._store()
        store.create("x", "content", created_by="alpha", actor_role=ROLE_ADMIN)
        with pytest.raises(PermissionError):
