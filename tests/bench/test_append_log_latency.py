...
Pass/fail thresholds (per design §1.4):
...
def test_append_log_latency_meets_budget(bench_segment_dir: Path) -> None:
    ...
    if p_max >= CEILING_BUDGET_MS:
        failures.append(f"max={p_max:.3f}ms ≥ {CEILING_BUDGET_MS}ms")
