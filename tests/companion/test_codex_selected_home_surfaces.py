...
def test_hooks_and_agents_install_into_selected_home(tmp_path: Path):
    ...
    hooks_path = hooks.install_hooks(codex_home=selected)
    agents_path = agents_md.install_agents_md(codex_home=selected)
...
def test_codex_commands_receive_selected_home(monkeypatch, tmp_path: Path):
    ...
    assert mcp_config.is_registered(selected)
    assert mcp_config.unregister(selected)
    assert hooks.ensure_hooks_feature_enabled(selected)
...
def test_mcp_registration_writes_selected_config(monkeypatch, tmp_path: Path):
    ...
    assert mcp_config.register({"TOKENPAK_COMPANION_PROFILE": "test"}, codex_home=selected)
...
def test_doctor_real_checks_report_and_use_every_selected_path(monkeypatch, tmp_path: Path, capsys):
    ...
    hooks.install_hooks(codex_home=selected)
    agents_md.install_agents_md(codex_home=selected)
...
def test_uninstall_cleans_only_selected_home_artifacts(monkeypatch, tmp_path: Path):
    ...
    monkeypatch.setattr(uninstall, "remove_mcp", lambda codex_home=None: True)
    monkeypatch.setattr(uninstall, "clean_rates_snapshot", lambda: (False, "absent"))
    monkeypatch.setattr(uninstall, "uninstall_skills", lambda: [])
...
def test_uninstall_reprobes_after_lease_before_mutation(monkeypatch, tmp_path: Path):
    ...
    calls: list[str] = []
    monkeypatch.setattr(uninstall, "remove_mcp", lambda *_a, **_k: calls.append("mcp"))
