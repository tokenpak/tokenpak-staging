...
def test_audit_internal_leakage_detects_planted_leak(tmp_path):
    ...
    pkg.mkdir()
    private_path = "/" + "/".join(
        ["home", "example", "vault", "03_" + "AGENT_PACKS", "worker", "queue", ""]
    )
    (pkg / "leak.py").write_text(f'"""Notes\n\nSee {private_path} for context."""\n')
    ...
    assert "private-home-path" in kinds or "vault-path" in kinds
