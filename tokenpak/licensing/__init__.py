...
def _license_path() -> Path:
    """Resolve the license file path through the TokenPak home-directory standard's resolver.
    ...
    Resolution order:
      ...
      2. ``<TOKENPAK_HOME>/license.json`` via ``tokenpak._paths.under``,
         ...
         legacy ``~/.tokenpak/`` per the TokenPak home-directory standard.

    Beta-1 QA-found regression fix: previously this hardcoded
    ...
    sandbox — a sandbox-escape + the TokenPak home-directory standard boundary violation in one.
    """
...
def activate(key: str, *, email: str = "") -> ActivationResult:
    ...
    once wired (Packet G-private, the Pro tier architecture).
