...
_CacheEntry = dict[str, str | float]
_ReferenceCache = dict[str, _CacheEntry]
_WireBlock = dict[str, object]


def _load_cache(cache_path: str) -> _ReferenceCache:
    p = Path(cache_path)
    if p.exists():
        try:
            return cast(_ReferenceCache, json.loads(p.read_text()))
        except (json.JSONDecodeError, OSError):
            return {}
    return {}


def _save_cache(data: _ReferenceCache, cache_path: str) -> None:
    p = Path(cache_path)
...
def _cache_get(ref: Reference, cache: _ReferenceCache) -> Optional[str]:
...
def _cache_put(ref: Reference, content: str, cache: _ReferenceCache) -> None:
...
def _prune_stale(cache: _ReferenceCache) -> _ReferenceCache:
    """Remove entries older than TTL."""
    now = time.time()
    return {
        k: v for k, v in cache.items() if now - float(v.get("fetched_at", 0)) <= _CACHE_TTL_SECONDS
    }
...
def _build_ephemeral_block(ref: Reference, content: str) -> _WireBlock:
...
def compile_with_refs(
    ...
    all_text = query + "\n" + "\n".join(cast(str, b.get("content", "")) for b in blocks)
    ...
    fetched_blocks: list[_WireBlock] = []
    if refs:
        to_fetch = []
        for ref in refs:
        ...
        if to_fetch:
            results: dict[str, tuple[Reference, str]] = {}
            ...
            for ref, content in results.values():
    ...
    regular_tokens = sum(cast(int, b.get("tokens", 0)) for b in blocks)
    remaining = budget - regular_tokens

    included_ephemeral: list[_WireBlock] = []
    for eb in fetched_blocks:
        block_tokens = cast(int, eb.get("tokens", 0))
