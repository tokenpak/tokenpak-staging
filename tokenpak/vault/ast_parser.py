    ...
    def _parse_python(self, content: str) -> list[ParsedNode]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, (ast.FunctionDef, ast.AsyncFunctionDef)):
                kind = "function" if not self._is_method(node, tree) else "method"  # type: ignore[arg-type]  # node is narrowed to FunctionDef | AsyncFunctionDef above; _is_method only accepts FunctionDef
                sig = self._python_signature(node, lines)  # type: ignore[arg-type]  # node is narrowed to FunctionDef | AsyncFunctionDef above; _python_signature only accepts FunctionDef
