...
def _resolve_adapter_bundle(script_dir):
    ...
    Primary layout: the script ships at
    ``tokenpak/integrations/openclaw/tokenpak-inject.sh`` and the bundle
    is its sibling under ``hooks/openclaw-adapter/``. Fallback: legacy
    repo-root layout where the install script lives at
    ``integrations/openclaw/tokenpak-inject.sh`` and the bundle lives
    under ``tokenpak/integrations/openclaw/hooks/openclaw-adapter/``.
    """
    candidates = [
