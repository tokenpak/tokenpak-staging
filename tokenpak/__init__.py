...
def __getattr__(name: str) -> Any:
    ...
    _lazy_map: dict[str, Callable[[], Any]] = {
        ...
        "CompressionEngine": lambda: (
            __import__(
                "tokenpak.compression.engines.base", fromlist=["CompactionEngine"]
            ).CompactionEngine
        ),
        "HeuristicEngine": lambda: (
            __import__(
                "tokenpak.compression.engines.heuristic", fromlist=["HeuristicEngine"]
            ).HeuristicEngine
        ),
        "get_engine": lambda: (
            __import__("tokenpak.compression.engines", fromlist=["get_engine"]).get_engine
        ),
        # Budgeting
        "Budgeter": lambda: (
            __import__("tokenpak.telemetry.budgeter", fromlist=["Budgeter"]).Budgeter
        ),
        "BudgetBlock": lambda: (
            __import__("tokenpak.telemetry.budget_allocator", fromlist=["BudgetBlock"]).BudgetBlock
        ),
        # Telemetry — CompletionTracker re-exported from tokenpak.telemetry
        "TelemetryCollector": lambda: (
            __import__(
                "tokenpak.telemetry.collector", fromlist=["TelemetryCollector"]
            ).TelemetryCollector
        ),
        "CacheManager": lambda: (
            __import__("tokenpak.cache.cache_manager", fromlist=["CacheManager"]).CacheManager
        ),
        "CompletionTracker": lambda: (
            __import__("tokenpak.telemetry", fromlist=["CompletionTracker"]).CompletionTracker
        ),
        # Token utilities
        "count_tokens": lambda: (
            __import__("tokenpak.telemetry.tokens", fromlist=["count_tokens"]).count_tokens
        ),
        # Packing
        "pack_prompt": lambda: (
            __import__("tokenpak.compression.pack", fromlist=["pack_prompt"]).pack_prompt
        ),
        "ContextPack": lambda: (
            __import__("tokenpak.compression.pack", fromlist=["ContextPack"]).ContextPack
        ),
        "PackBlock": lambda: (
            __import__("tokenpak.compression.pack", fromlist=["PackBlock"]).PackBlock
        ),
        "CompiledResult": lambda: (
            __import__("tokenpak.compression.pack", fromlist=["CompiledResult"]).CompiledResult
        ),
        # Registry
        "Block": lambda: __import__("tokenpak.core.registry", fromlist=["Block"]).Block,
        "BlockRegistry": lambda: (
            __import__("tokenpak.core.registry", fromlist=["BlockRegistry"]).BlockRegistry
        ),
        # Reports
        "Action": lambda: __import__("tokenpak.compression.report", fromlist=["Action"]).Action,
        "CompileReport": lambda: (
            __import__("tokenpak.compression.report", fromlist=["CompileReport"]).CompileReport
        ),
        "Decision": lambda: (
            __import__("tokenpak.compression.report", fromlist=["Decision"]).Decision
        ),
        # CLI
