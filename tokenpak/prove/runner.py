...
def _build_arms(scenario: Scenario) -> list[ArmConfig]:
    ...
    if scenario.matrix:
        ...
        for entry in scenario.matrix:
            arms.append(
                ArmConfig(
                    name=entry.get(
                        "name", f"{entry.get('provider', '?')}/{entry.get('model', '?')}"
                    ),
    ...
    return [
        ArmConfig(
            name="Direct API", platform="api", provider=scenario.provider, model=scenario.model
        ),
        ArmConfig(
            name="With TokenPak",
            platform="proxy",
...
def run_proof(
    ...
    for i, arm in enumerate(arms):
        ...
        print(
            f"    [{i + 1}] {arm.name:24s}  {arm.platform}/{arm.provider}/{arm.model}{tp}",
            file=sys.stderr,
        )
    ...
    if display:
        print(
            "\n  Live display still running — close with: tmux kill-session -t tokenpak-prove",
            file=sys.stderr,
        )
