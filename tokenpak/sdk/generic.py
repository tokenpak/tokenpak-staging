...
_log = logging.getLogger("tokenpak.sdk.generic")
    ...
    def send(self, prepared_request: dict[str, Any]) -> dict[str, Any]:
        ...
        if not _REQUESTS_AVAILABLE:
            raise TokenPakAdapterError(
                "GenericAdapter.send: 'requests' package is required. "
        ...
        if resp.status_code in (401, 403):
            raise TokenPakAuthError(
                f"GenericAdapter.send: authentication failed (HTTP {resp.status_code}).",
        ...
        if not resp.ok:
            try:
                ...
                err_body = resp.text
            raise TokenPakAdapterError(
                f"GenericAdapter.send: proxy returned HTTP {resp.status_code}.",
                status_code=resp.status_code,
                raw=err_body,
            )
