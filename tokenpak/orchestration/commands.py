...
# Known TokenPak subcommands. When an action's first token is one of these (and
# the action is not a filesystem path), it is prefixed with ``tokenpak`` so a
# stored action of ``status`` runs ``["tokenpak", "status"]``. Kept in sync with
# the CLI's top-level command set; an unknown first token is treated as an
# external command and run as-is.
...
def parse_trigger_action(
    action: str,
    *,
    subcommands: frozenset = TOKENPAK_SUBCOMMANDS,
    warn: bool = True,
) -> CommandAction:
