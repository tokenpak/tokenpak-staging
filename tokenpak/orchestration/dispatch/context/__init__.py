"""TokenPak Dispatch — context assembly (Dispatch contract §5.9).

This subpackage hosts the ``ContextProvider`` interface plus the two
implementations the Dispatch contract describes:
...
* :class:`PaidContextProvider` — a stub that raises ``NotImplementedError``.
  ...
  instance, not a rewrite (Dispatch contract §5.9 "Pro path").
...
cap from Spend Guard through TIP (Dispatch contract §8). This module performs
