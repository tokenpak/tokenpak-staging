...
* :class:`LocalContextProvider` — the OSS v0.1-alpha provider. Deterministic,
  no LLM, no network, no Std 32 Pak dependency. It assembles a
...
* :class:`PaidContextProvider` — a stub that raises ``NotImplementedError``.
  ...
  one (Std 32 §1.3): the Pro path is a later *swap* of the provider instance,
  not a rewrite (Standards Delta v0 §5.9 "Pro path").

The token budget inherits the Std 29 Spend Guard cap. In v0.1-alpha that cap is
