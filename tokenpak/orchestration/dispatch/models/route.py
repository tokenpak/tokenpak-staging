"""DispatchRoute record (Dispatch contract §4.3)."""
...
class RouteStation(DispatchBaseModel):
    """A single station definition within a route (Dispatch contract §4.3).
...
class RouteTriggers(DispatchBaseModel):
    """Route trigger declaration (Dispatch contract §4.3)."""

    intents: list[str] = Field(default_factory=list)


class RouteRetryPolicy(DispatchBaseModel):
    """Route-level retry policy (Dispatch contract §4.3)."""
...
class RouteDelivery(DispatchBaseModel):
    """Route-level delivery package composition flags (Dispatch contract §4.3)."""
...
class DispatchRoute(DispatchBaseModel):
    """A named, versioned workflow route (Dispatch contract §4.3)."""
