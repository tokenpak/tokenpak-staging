"""DispatchReceipt record (Dispatch contract §4.7)."""
...
class ReceiptStation(DispatchBaseModel):
    """Per-station summary row on a receipt (Dispatch contract §4.7)."""
...
class ReceiptDecision(DispatchBaseModel):
    """Per-decision summary row on a receipt (Dispatch contract §4.7)."""
...
class ReceiptEffect(DispatchBaseModel):
    """Per-effect summary row on a receipt (Dispatch contract §4.7)."""
...
class ReceiptTelemetry(DispatchBaseModel):
    """Aggregated telemetry block on a receipt (Dispatch contract §4.7)."""
...
class DispatchReceipt(DispatchBaseModel):
    """Delivery receipt summarizing a completed run (Dispatch contract §4.7)."""
