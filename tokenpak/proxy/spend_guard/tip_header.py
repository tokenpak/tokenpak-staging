...
def _set_deterministic(d: TIPDirective, v: Optional[str]) -> None:
    """``[TIP: deterministic=on]`` — reproducible eval mode (PROP4-1B).
    ...
    PROP4-1B contract, unsupported deterministic fields are NEVER silently
    stripped.
    """
    if v is None or v in ("on", "true", "1", "yes"):
...
# ---------------------------------------------------------------------------
# Request fingerprint (deterministic eval mode — PROP4-1B)
# ---------------------------------------------------------------------------
