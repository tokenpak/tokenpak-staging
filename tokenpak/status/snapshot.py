...
"""Render-agnostic status provider and ``StatusSnapshot`` data contract.

This module deliberately ships two layers:
...
- **No numeric savings claim** lives on this contract until that value is
...
Dependencies: stdlib + ``tokenpak._paths`` only. ``__all__`` is intentionally
empty: this provider is internal plumbing, not a frozen public-API surface.
Consumers import by explicit path.
"""
...
class StatusSnapshot:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        Deliberately carries **no** savings field until that value is governed and
...
def build_status_snapshot(
    *,
    ...
    without a verified session-correlation key. When their evidence is absent
    they stay ``unknown`` -- never fabricated.
    """
