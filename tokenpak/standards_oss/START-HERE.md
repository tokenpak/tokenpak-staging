...
compliance framework, and not a certification. See `GOVERNANCE.md` §7 for exactly what "conformance"
...
**You are adopting this for work you control.** Read §2, pick your profiles (§3), declare your
capabilities (§4), then follow the reading path in §6.

**You are joining work someone else already governs.** Read `GOVERNANCE.md` §2–§4 (terms, authority
model, protected actions), then the compiled profile for the work you are joining
(`profiles/README.md` §4 explains how to read it). Do not re-select profiles — the operator owns
that choice.
...
|---|---|---|
| `starter` | You are beginning, or the work is small and personal | The eight foundations in §6 — truth, risk, records, acceptance, spend, credentials, incidents |
...
| `multi-agent-fleet` | Several executors work concurrently on shared resources | Everything, with concurrency and shared-resource control mandatory |
...
| `bounded-autonomous` | Work proceeds on evidence with batched notification instead of approval. **Bounded** is load-bearing: the protected actions in `GOVERNANCE.md` §4 stay human in every profile. There is no unbounded profile and there will not be one. |
...
3. Fill in the adoption file (§4) — capabilities, owner, budgets
4. Enumerate YOUR protected actions           GOVERNANCE.md §4 — the concrete list for your work
...
|---|---|---|---|
| 1 | **Authority and modes** | Configure now | `GOVERNANCE.md` §2–§6 |
| 2 | **Truth and evidence** | Read now | `core/truth-and-evidence.md` |
| 3 | **Risk, reversibility, protected actions** | Read now, then configure §4 list | `core/risk-and-protected-actions.md` |
...
disagree, that is a defect to fix, never a precedence question to resolve. See `GOVERNANCE.md` §8.
...
Your copy is yours. `GOVERNANCE.md` §9 gives you the lifecycle: propose → decide → record →
