...
class TestConfigValidation:
    def test_config_loader_failure_warns_and_uses_default_caps(self, monkeypatch, capsys):
        import tokenpak.core.config_loader as config_loader

        def fail_load_config():
            raise ValueError("bad config parse")

        monkeypatch.setattr(config_loader, "load_config", fail_load_config)

        cfg = load_config()

        captured = capsys.readouterr()
        assert "WARN" in captured.err
        assert "bad config parse" in captured.err
        assert cfg.default_context_window_percent == 90
        assert cfg.hard_stop_context_window_percent == 100
